...
/**
 * @author yangnana 2014-11-22 实现左滑显示删除的ListView
 */
public class SwipeListView extends ListView {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        float lastX = ev.getX();// 获取事件处的x坐标
        float lastY = ev.getY();// 获取事件处的y坐标
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:// 鼠标按下事件
                mIsHorizontal = null;
                mFirstX = lastX;
                mFirstY = lastY;
                int motionPosition = pointToPosition((int) mFirstX, (int) mFirstY);

                System.out.println("SwipeListView   motionPosition=" + motionPosition);
                if (motionPosition >= 0) {
